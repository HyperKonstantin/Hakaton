...
class Button:
    def __init__(self, passive_img, active_img, callback, scale = 1):
        ...
        self.pass_img = pg.transform.scale(passive_img, (int(width * scale), int(height * scale)))
        self.act_img = pg.transform.scale(active_img, (int(width * scale), int(height * scale)))
